...
def test_home_view_function_is_correct(db):
    view = resolve(reverse('recipes:home'))
    assert view.func is views.home
    assert view.func is views.home


def test_home_template_shows_no_recipes_found_if_no_recipes(db, client):
...
def test_home_template_uses_recipes_template(db, client):
...
def test_home_template_doesnt_load_non_published_recipes(db, client):
...
def test_category_template_loads_recipes(db, client):
...
def test_category_view_function_is_correct(db):
    view = resolve(reverse('recipes:category', kwargs={'category_id': 1}))
    assert view.func is views.category


def test_category_template_doesnt_load_non_existent_category(db, client):
    RecipeFactory(is_published=False)
    response = client.get(reverse('recipes:category', kwargs={'category_id': 1}))
...
def test_detail_view_function_is_correct(db):
    view = resolve(reverse('recipes:recipe', kwargs={'id': 1}))
    assert view.func is views.recipe


def test_detail_template_loads_correct_recipe(db, client):
...
def test_detail_template_doesnt_load_non_published_recipe(db, client):
    RecipeFactory(is_published=False)
...
def test_search_uses_correct_view_function(db):
    url = reverse('recipes:search')
    resolved = resolve(url)
    assert resolved.func is views.search


def test_search_can_find_recipe_by_title(db, client):
